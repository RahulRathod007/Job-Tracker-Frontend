...
// import { getAllJobs, searchByLocation } from '../services/api'
...
//       // Log to console so you can inspect in browser DevTools
//       console.log('API response:', response)
//       console.log('response.data:', response?.data)
//       console.log('type:', typeof response?.data, Array.isArray(response?.data))
...
//         setJobs(data)
//         if (data.length === 0) {
//           setDebugInfo('Backend returned an empty array. Post a job first using an employer account.')
//         }
//       } else {
//         // Backend returned something unexpected
//         setJobs([])
//         setDebugInfo(`Backend returned: ${JSON.stringify(data).slice(0, 200)}`)
...
//       // Show the most helpful error message possible
//       if (err.code === 'ERR_NETWORK' || err.message === 'Network Error') {
//         setError('Cannot reach backend. Is Spring Boot running on http://localhost:8080?')
//       } else if (err.response?.status === 403) {
//         setError('403 Forbidden - The backend rejected the request. Check CORS settings.')
//       } else if (err.response?.status === 401) {
//         setError('401 Unauthorized - Backend requires authentication for this endpoint.')
//       } else if (err.response?.status === 404) {
//         setError('404 - Endpoint /api/jobs/all not found. Check your backend is running correctly.')
//       } else if (err.response?.status >= 500) {
//         setError(`Server error ${err.response.status} - Check your Spring Boot console for exceptions.`)
//         setDebugInfo(err.response?.data?.message || err.response?.data || '')
//       } else {
...
//                   <i className="bi bi-geo-alt" style={{ color: 'var(--text-muted)', padding: '0 4px 0 10px', alignSelf: 'center' }}></i>
//                   <input
//                     type="text"
//                     placeholder="Search by location e.g. Remote, Bangalore"
//                     value={searchInput}
...
//           {/* ── States: error / loading / empty / results ── */}
//           {error ? (
//             <div style={{
...
//               }}>
//                 <i className="bi bi-wifi-off" style={{ color: 'var(--accent-2)' }}></i>
//               </div>
//               <h4 style={{ color: 'var(--accent-2)', marginBottom: '10px' }}>Could not load jobs</h4>
//               <p style={{ color: 'var(--text-muted)', maxWidth: '480px', margin: '0 auto 8px', fontSize: '0.92rem' }}>
//                 {error}
//               </p>
//               {debugInfo && (
//                 <p style={{
...
//                   : debugInfo || 'No jobs posted yet. An employer needs to post jobs first.'}
//               </p>
//               {activeSearch && (
