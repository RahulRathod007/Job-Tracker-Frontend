...
export default function Register() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      loginUser(res.data)   // ← saves tokens, logs in directly — no redirect to /login
      navigate('/dashboard')
    } catch (err) {
      const data = err?.response?.data
      if (typeof data === 'object' && data !== null && !data.error) setErrors(data)
      else setErrors({ _global: data?.error || 'Registration failed. Please try again.' })
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    ...
    try {
      const res = await googleAuth({ credential: credentialResponse.credential, role: formData.role })
      loginUser(res.data)
      navigate('/dashboard')
    } catch (err) {
      setErrors({ _global: err?.response?.data?.error || 'Google sign-up failed.' })
              ...
              <div className="jp-form-card fade-in" style={{ maxWidth: '520px', margin: '0 auto' }}>
                ...
                <div style={{ marginBottom: '24px' }}>
                  ...
                  <div className="role-toggle">
                    {[
                      { value: 'JOBSEEKER', icon: '🧑‍🎓', label: 'JOBSEEKER',  desc: 'Register as Jobseeker' },
                      { value: 'EMPLOYER',  icon: '🧑‍💻', label: 'EMPLOYER', desc: 'Register as Employee' },
                    ].map(r => (
                      <div key={r.value} className={`role-option ${formData.role === r.value ? 'selected' : ''}`}
                ...
                <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '4px' }}>
                  {gLoading ? (
                    ...
                    <GoogleLogin
                      ...
                      onError={() => setErrors({ _global: 'Google sign-in failed.' })}
                      width="460" theme="filled_black" shape="rectangular" text="signup_with"
                    />
                  )}
                </div>
